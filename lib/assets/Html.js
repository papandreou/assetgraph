...
extendWithGettersAndSetters(Html.prototype, {
    ...
    get text() {
        var self = this;
        return Object.keys(this._templateReplacements).reduce(function (text, key) {
                    ...
                    if (!type || type === 'text/javascript' || type === 'text/coffeescript') {
                        ...
                        if (node.hasAttribute('data-main')) {
                            ...
                            if (this.assetGraph && this.assetGraph.requireJsConfig) {
                                if (this.assetGraph.requireJsConfig.baseUrl) {
                                    if (!/^\/|\.js(?:\?.*)?$/.test(node.getAttribute('data-main'))) {
                                        url = this.assetGraph.resolveUrl(this.assetGraph.requireJsConfig.baseUrl, url);
                                    }
                                } else {
                                    var urlToDataMainFile = this.assetGraph.resolveUrl(this.nonInlineAncestor.url, url);
                                    this.assetGraph.requireJsConfig.baseUrl = urlToDataMainFile.replace(/[^\/]*$/, '');
                                    if (!/^\/|\.js(?:\?.*)?$/.test(node.getAttribute('data-main'))) {
                                        url = urlToDataMainFile.replace(/^.*\//, '');
                                    }
                                }
                            }
                            ...
                            //     }
                            // }
                            // // if it's a relative path to the data-main file, prepend it with the basePath
                            // if (this.assetGraph && this.assetGraph.requireJsConfig && this.assetGraph.requireJsConfig.baseUrl && !/^\/|\.js(?:\?.*)?$/.test(node.getAttribute('data-main'))) {
                            //     url = this.assetGraph.resolveUrl(this.assetGraph.requireJsConfig.baseUrl, url);
                            // }
            ...
            for (var i = 0 ; i < node.childNodes.length ; i += 1) {
                ...
                if (childNode.nodeType === childNode.ELEMENT_NODE) {
                    ...
                    if (/^\s*\/?(?:ko|hz)(?:\s|$)/.test(childNode.nodeValue)) {
                        // Knockout.js or Htmlizer containerless binding start or end marker. Remove superfluous whitespace if removeWhiteSpace is true:
                        if (removeWhiteSpace) {
